...
Readings: (i) negotiation arms cannot discriminate without histories --- arena gap documented; (ii) bargaining shows a small inversion (L0/L1 $>$ L2/L3): the tracker overfits $\hat\delta$ to two deterministic opponents; (iii) persuasion deltas fall within noise because both arms include the post-F6 buyer. After the arena gaps were fixed (negotiation histories fed; named opponents with per-arm profile stores; n$=120$/arm), the bargaining inversion replicated (L0/L1 0.243 $\pm$ 0.016 vs L2/L3 0.209 $\pm$ 0.014, SE over games; pooled difference $+3.4$pp, 95\% CI $[-0.9, +7.6]$pp, Welch --- not individually significant). A live paired A/B then tested the same layer in production over a 19-hour alternation window: tracker-ON 0.498 $\pm$ 0.010 (n=246 agreement games) vs tracker-OFF 0.488 $\pm$ 0.013 (n=173); difference $+1.0$pp favoring ON, 95\% CI $[-2.2, +4.2]$pp (Welch) --- no detectable difference. Re-analysis note: an earlier 72-game subset of this window suggested a $+13.7$pp flip; the complete window does not support it, and we retract that claim. Both CIs are consistent with zero: the tracker is neither demonstrably harmful nor helpful at achievable sample sizes. We retained the theoretically-motivated tracker, and flag the fragility of small-sample ablation reads (both our own $n{=}40$ subset and the arena estimate) as an external-validity caution for local ablation methodology.
