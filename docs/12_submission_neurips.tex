...
\emph{Scripted arena} (opponents fixed, 120 games/arm, paired seeds, pooled 240 vs 240): tracker-OFF 0.243 $\pm$ 0.016 vs tracker-ON 0.209 $\pm$ 0.014 mean pot share; difference $+3.4$pp favoring OFF, 95\% CI $[-0.9, +7.6]$pp (Welch $t=1.59$, df$\approx473$) --- directionally harmful, not individually significant.

\emph{Live field} (30-min session alternation over a 12-hour window; all agreement games in the window): tracker-ON 0.498 $\pm$ 0.010 (n=246) vs tracker-OFF 0.488 $\pm$ 0.013 (n=173); difference $+1.0$pp favoring ON, 95\% CI $[-2.2, +4.2]$pp (Welch $t=0.61$, df$\approx360$) --- no detectable difference.

Both CIs are consistent with zero and with each other. A post-hoc power check: at the observed per-game variability, 80% power for a $3.4$pp difference would require ${\sim}350$--$800$ games per arm; we ran 240/240 (arena, ${\sim}36$% power) and 246/173 (live, ${\sim}9$% power). The methodological lesson survives, sharpened: a scripted-arena ablation produced a $3.4$pp point estimate whose sign does not establish a field effect, and the field experiment was underpowered for effects of this size. We retained the theoretically-motivated tracker (its live point estimate is non-negative), and we caution against deploying or removing components on the basis of small local ablations alone.
